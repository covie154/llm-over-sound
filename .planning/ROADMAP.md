...
- [x] 03-02-PLAN.md — CT AP structured variant, CT thorax, and US HBS templates
...
### Phase 7: Replace ggwave with minimodem (both sides)

**Goal:** The data-over-sound transport is minimodem FSK on both the AHK frontend and the Pi Python backend, interoperating over the same audio link with single-frame CRC32 integrity and configurable end-to-end baud — every layer above transport (GUI, message protocol, LZNT1/Base62 compression, message IDs) preserved.
**Requirements**: none registered in REQUIREMENTS.md; derived from CONTEXT.md locked decisions (TX-01, RX-01, WINMM-01, FFTW-01, CRC-01, API-01, CFG-01 per RESEARCH.md)
**Depends on:** Phase 6
**Plans:** 5 plans

Plans:

- [ ] 07-01-PLAN.md — minimodem-wrapper scaffold + minimodem.c decomposition (ctx struct / mm_build_config / mm_tx_bytes / mm_rx_step) + WinMM backend + dependency-free DLL build
- [ ] 07-02-PLAN.md — Public API bodies + background RX thread + newline-framed queue; Wave 0 gate: byte-exact loopback self-test + cross-language CRC32 agreement vector
- [ ] 07-03-PLAN.md — AHK frontend: load minimodem_simple.dll, BAUD_RATE, Crc32Str, single-frame + crc send/receive with full retransmit
