 ...
 *
 * TRIMMED for the minimodem_simple v1 transport: only the 8-N-1 ASCII
 * databits codec is vendored (databits_ascii.c). The upstream baudot /
 * callerid / uic / binary codecs are NOT vendored, so the corresponding
 * declarations and the `#include "baudot.h"` (which pulled in the baudot
 * module) have been removed. The two inline bit helpers below are used by
 * mm_rx_step and are kept verbatim from upstream.
 */
