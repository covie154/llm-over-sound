...
/*
 * minimodem-wrapper vendored copy.
 *
 * Backend selection here is driven by compile-time defines:
 *   - USE_WINAUDIO  : Windows WinMM backend (simpleaudio-winmm.c)
 *   - USE_ALSA      : Linux ALSA backend    (simpleaudio-alsa.c)
 *   - USE_PULSEAUDIO: Linux PulseAudio      (simpleaudio-pulse.c)
 * The CMake build defines USE_WINAUDIO on Windows and USE_ALSA/USE_PULSEAUDIO
 * on Linux. The sndfile/sndio/benchmark backends are NOT vendored.
 */
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
#ifdef USE_WINAUDIO
	case SA_BACKEND_WINAUDIO:
	    sa->backend = &simpleaudio_backend_winaudio;
	    break;
#endif
