...
## Overview
This system uses minimodem (FSK data-over-sound) to pipe messages between an AutoHotkey v2
...
frontend. The draft is transmitted over minimodem to the Python backend, which classifies
the study type, selects the appropriate report template, extracts and maps findings to
anatomical fields using the LLM, and transmits the formatted report back over minimodem
...
- **AHK Frontend** (`AHK/main_dll.ahk`): Windows GUI that captures the radiologist's
  ...
  minimodem_simple DLL. Receives the formatted report back, presents __NOT_DOCUMENTED__
  fields for review, and handles final report approval. Uses LZNT1/XPRESS compression
  (via ntdll) and Base64 encoding for payload efficiency [2].
- **Python Backend** (`python-backend/backend.py`): Runs on a Raspberry Pi connected to
  the user's PC. It has not yet been determined if the connection is a speaker-microphone or wired audio connection. Listens for minimodem FSK signals via subprocess,
  decodes messages, orchestrates the LLM-powered report formatting pipeline, and
  transmits results back over minimodem [3]. Connects to the LLM over the network.
- **minimodem Wrapper** (`minimodem-wrapper/`): C DLL wrapper extracting minimodem's
  FSK core, paired with PortAudio for audio I/O. Built with CMake.
- **ggwave Wrapper (archived)** (`archive/ggwave-wrapper/`): Previous C/C++ DLL wrapper
  around the ggwave library. Archived after migration to minimodem.
- **ggwave** (`ggwave/`): The upstream ggwave library (data-over-sound). Kept as reference.
...
- minimodem uses continuous FSK modulation — there is no per-frame payload limit as
  with ggwave. Data is streamed as a continuous bitstream over audio.
- Messages are compressed above COMPRESSION_THRESHOLD (100 chars) using LZNT1 [2].
- CHUNK_DATA_SIZE is configurable (~1500 chars default). Chunking is retained for
  message IDs, sequencing headers (message ID, chunk index "ci", chunk count "cc"),
  and retransmission support.
- Each JSON chunk is terminated with a newline (`\n`) as the message delimiter.
- Baud rate is configurable, starting at 1200 (Bell 202 compatible).
...
- All data must travel over the audio channel. No shared filesystem or secondary
  network channel between frontend and backend.
...
- All logging is file-based: `minimodem_log.txt` for AHK [2], `backend_log.txt` for
  Python [3].

## Build
- The minimodem wrapper is built via CMake from `minimodem-wrapper/`. Dependencies:
  FFTW3f and PortAudio, installed via MSYS2. Output goes to `build/`.
